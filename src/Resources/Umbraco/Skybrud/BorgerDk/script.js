...
$(document).ready(function () {
    ...
    $('.propertyItemContent>.borgerDkMicroArticles').each(function (i, e) {

        var panel = $(e);

        var options = {
            propertyId: parseInt(panel.attr('data-propertyid')),
            municipalityId: parseInt(panel.attr('data-municipalityid')),
            limit: parseInt(panel.attr('data-limit')),
            allowBlocks: panel.attr('data-allow-blocks') == 'true'
        };

        var input = $('.borgerDkUrl', panel);
        var summary = $('.borgerDkSummary', panel);
        var loader = $('.borgerDkLoader', panel);
        var content = $('.borgerDkContent.borgerDkMicroArticles', panel);
        var content2 = $('.borgerDkContent.borgerDkBlocks', panel);
        var errors = $('.borgerDkErrors', panel);
        var example = $('.borgerDkExample', panel);
        ...
        function getMicroArticles(url, cache) {
            ...
            $.getJSON('/base/BorgerDk/GetMicroArticles.aspx', { url: url, cache: cache, municipalityId: options.municipalityId }, function (json) {
                getArticleCallback(json, selectedUrl, url);
            });
        ...
        function getArticleCallback(json, lastUrl, newUrl) {
            ...
            if (json.meta.code == 200) {
                ...
                var list = $('<ul class="borgerDkContentList"/>').appendTo(content);
                var list2 = $('<ul class="borgerDkContentList"/>').appendTo(content2);

                $('<li class="borgerDkListHeader"><span>Vælg</span><span>Beskrivelse</span></li>').appendTo(list);
                $('<li class="borgerDkListHeader"><span>Vælg</span><span>Beskrivelse</span></li>').appendTo(list2);
                ...
                $.each(json.data.elements, function (i, e) {
                    ...
                    var ch = $('<input/>', {
                        id: e.id + '_Data' + options.propertyId,
                        type: options.limit == 1 ? 'radio' : 'checkbox',
                        name: 'microArticles_' + options.propertyId
                    }).attr('checked', selected.contains(e.id)).appendTo(li);

                    var lb = $('<label for="' + e.id + '_Data' + options.propertyId + '">' + e.text + ' <span class="borgerDkItemClass">(mikroartikel)</span></label>').appendTo(li);
                    var ln = $('<a class="borgerDkItemTrigger" href="#">Vis dette indhold</a>').appendTo(li);
                ...
                if (options.allowBlocks) {
                    $.each(json.data.other, function (i, e) {

                        var li = $('<li/>').appendTo(list2);
                        ...
                        var lb = $('<label for="' + e.id + '_Data' + options.propertyId + '">' + e.text + ' <span class="borgerDkItemClass">(' + e.type + ')</span></label>').appendTo(li);
                        var ln = $('<a class="borgerDkItemTrigger" href="#">Vis dette indhold</a>').appendTo(li);
                ...
                $('input:checkbox,input:radio', panel).change(function () {

                    var selected = [];

                    $('input:checkbox,input:radio', panel).each(function (i, e) {
                        if (e.checked) selected.push(e.id.split('_')[0]);
                    });
        ...
        function onChange() {
            ...
            if (url == selectedUrl) return;
            if (isValidUrl(url)) getMicroArticles(url, true);
